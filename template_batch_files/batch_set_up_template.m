...
parcs.pdir(4).name='Tian_Subcortex_S2_3T_FSLMNI152_1mm';
...
flags.T1.parc=1;
    ...
    configs.T1.subcortUser = 1; % 0 = default FSL, 1 = user-provided
...
% wish to only process select sessions, set them via epiMin and epiMax.
    configs.EPI.epiMin = 1; % minimum scan index (e.g., 1)
    configs.EPI.epiMax = 6; % maximum scan index (e.g., 6)
...
flags.DWI.dcm2niix = 0; % dicom to nifti coversion
    ...
    configs.DWIdcm(1).phase = [0 0 0 0]; % define here: array gets read in from dicom headers
