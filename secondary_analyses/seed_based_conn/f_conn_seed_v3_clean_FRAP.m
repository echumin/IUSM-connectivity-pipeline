...
addpath(genpath('/usr/local/IUSM-connectivity-pipeline-jenya-wip/connectome_scripts'))
...
% paths.data = '/bgodata02/CONN/BAD/subjects';
paths.data = '/bgodata02/CONN/FRAP/subjects';
paths.timing = '/bgodata01/FRAP';
timingfile = 'fMRI_timings_v3.csv';

% Creating a directory for results
paths.results = fullfile(paths.data,'..','seed_results');
...
% ScanID = 'EPI1'; % 'REST' or 'EPI1' or 'whatever'
niiOut = 'AROMA';
niiInp = 'AROMA/aCompCorr';
GSregID = 'Gs4'; % 'GSreg_yes' or 'GSreg_no' (GS regression on/off)
PCAID = 'pca5';  % 'pca0', 'pca1', 'pca3', or 'pca5' (see connectome scripts)
% paths.resultscp = fullfile(paths.results,strcat(ScanID,'_',niiOut,'_',GSregID,'_',PCAID));
% if ~exist(paths.resultscp,'dir')
%     mkdir(paths.resultscp)
% end
ijmin = 6; % EPI index (e.g. first EPI to process)
ijmax = 6; % EPI index 6(e.g., 6 to loop over 6 scans)

% List of subject numbers
% slist = [ 160; 171; 173; 182; 189; 196; 197; 201; 203; 204; 215; 217; ...
%     219; 220; 221; 231; 235; 249; 256; 270; 277; 280; 286; 312; 320; ...
%     326; 343; 349; 353; 355; 356; 363; 368; 380; 401; 406; 418; 437; ...
%     446; 461 ];
slist = [ 14; 15; 16; 17; 19; 24; 26; 27; 30; 34; 35; 40; 44; 52; 53 ];
...
seedCoordList = ...
    '/usr/local/IUSM-connectivity-pipeline-jenya-wip/secondary_analyses/seed_based_setup/testing_seeds.txt';
...
seedLabel = 'R_AIC';
seedImage = ...
    '/usr/local/IUSM-connectivity-pipeline-jenya-wip/secondary_analyses/seed_based_setup/BAD2/R_AIC_Cho-Bl_2mm.nii';
...
configs.dropTR = 15; % defaults to 15

% Calculate correlation for a subset of time points 
configs.block = ' '; % Comment out either USER-SPECIFIED or BLOCK-NAME lines
% USER-SPECIFIED: the next three lines should be used together
% configs.block = 'UserDefined'; 
% configs.firstTR = 10; % First volume for the FC evaluation
% configs.lastTR = 200; % Last volume for the FC evaluation
% BLOCK-NAME: the next two lines should be used together
configs.block = 'Empty2'; % options: 'Empty1', 'AddFuture', 'RecFuture','Empty2'
HRFlag = 4; % HRF peak lag (in seconds) for task design
...
for i=1:length(list.subjects)
    for ij = ijmin:ijmax % scans        
        ScanID=strcat('EPI',num2str(ij));
        paths.resultscp = fullfile(paths.results,strcat(ScanID,'_',niiOut,'_',GSregID,'_',PCAID));
        if ~exist(paths.resultscp,'dir')
            mkdir(paths.resultscp)
        end
        ...
        paths.GSPCA = fullfile(paths.EPI,niiInp);
        ...
        [EvtTime]=readtiming(paths,timingfile);
        
        % Read in the EPI 4D volume for the subject
        fileNii = strcat('7_epi_aroma_',GSregID,'_',PCAID,'.nii.gz');
        if exist(fullfile(paths.GSPCA,fileNii))
            subjNii = MRIread(fullfile(paths.GSPCA,fileNii));
            sentence = sprintf('fslnvols %s',fullfile(paths.GSPCA,fileNii));
            [status,result] = system(sentence);
            nvols = str2num(result);
            sentence = sprintf('fslinfo %s |grep -e ''pixdim4''|head -1 |tr -d ''pixdim4''',fullfile(paths.GSPCA,fileNii));
            [status,result] = system(sentence);
            TR = str2num(result)/1000;
            if flag_block == 0
                nvolmin = configs.dropTR+1; 
                nvolmax = nvols-configs.dropTR;
            elseif flag_block == 1
                nvolmin = round((HRFlag + EvtTime.Empty1.onset)/TR);
                nvolmax = nvolmin + round(EvtTime.Empty1.dur/TR);
            elseif flag_block == 2
                nvolmin = round((HRFlag + EvtTime.AddFuture.onset)/TR);
                nvolmax = nvolmin + round(EvtTime.AddFuture.dur/TR);
            elseif flag_block == 3
                nvolmin = round((HRFlag + EvtTime.RecFuture.onset)/TR);
                nvolmax = nvolmin + round(EvtTime.RecFuture.dur/TR);
            elseif flag_block == 4
                nvolmin = round((HRFlag + EvtTime.Empty2.onset)/TR);
                nvolmax = nvolmin + round(EvtTime.Empty2.dur/TR);             
            elseif flag_block == 5
                nvolmin = configs.firstTR;
                nvolmax = configs.lastTR;
            end
            % ensure BOLD volume range between 1 and nvols
            nvolmin = max(1,nvolmin); % lowest volume
            nvolmax = min(nvolmax,nvols); % highest volume 
        else
            warning('Does not exist')
        end
        % Drop early and late volumes
        subjVOL = subjNii.vol(:,:,:,nvolmin:nvolmax);clear subjNii;
        ...
        subjPARC = MRIread(fullfile(paths.EPI,'rT1_GM_parc_schaefer300_yeo17_clean.nii.gz'));
...
%                 end
                ...
                if configs.block == ' '
                    ...
                    MRIwrite(Temp,fullfile(paths.seed,...
                    sprintf('conn_%s_%s_wholebrain_epi.nii.gz',seedLabel,configs.block)));
                end
                % ------------------Seed-Only ---------------%
                S.vol = zeros(size(subjMaskGM));
                S.vol = reshape(S.vector,size(S.vol));
                ...
                if configs.block == ' '
                    ...
                    fileOut = fullfile(paths.seed,...
                    sprintf('conn_%s_%s_wholebrain_MNI.nii.gz',seedLabel,configs.block));
                end
                % Transformation field and matrix to MNI space.
                fileWarp = fullfile(paths.reg,'T12MNI_warp.nii.gz');
                fileMat = fullfile(paths.EPI,'epi_2_MNI_final.mat');
                ...
                if configs.block == ' '
                    ...
                    fileIn = fullfile(paths.seed,...
                        sprintf('conn_%s_%s_wholebrain_MNI.nii.gz',seedLabel,configs.block));
                    fileOut = fullfile(paths.resultscp,...
                        sprintf('%s_conn_%s_%s_wholebrain_MNI.nii.gz',...
                        list.subjects(i).name,seedLabel,configs.block));
                end
                sentence = sprintf('cp %s %s',fileIn,fileOut);
                [~,result]=system(sentence);
                ...
                if configs.block == ' '
                    ...
                    fileIn = fullfile(paths.resultscp,...
                        ...
                        list.subjects(i).name,seedLabel,configs.block));
                    fileSmoothed = fullfile(paths.resultscp,...
                        sprintf('%.0ffwhm_%s_conn_%s_%s_wholebrain_MNI.nii.gz',...
                        configs.fwhm,list.subjects(i).name,seedLabel,configs.block));                   
                end
                % Smooth the Images
                sentence = sprintf('fslmaths %s -s %.8f %s',fileIn,configs.sigma,fileSmoothed);
                [~,result]=system(sentence);
