...
if flags.DWI.dcm2niix == 1
    for nscan=1:nscanmax % 1 or 2 DWI scans
        ...
        if length(dir(paths.DWI.DCM))<= 2 %( '.' '..' are first 2 returns)
            ...
            sentence = sprintf('%s/connectome_scripts/dcm2niix/dcm2niix -f %s -o %s -v y %s',paths.dcm2niix,fileNii,paths.DWI.dir,paths.DWI.DCM);
            [~,result] = system(sentence);
            % save verbose output
    ...
    elseif nscanmax == 1 && length(dir(paths.DWI.dcmPA))<= 2 %( '.' '..' are first 2 returns)
        ...
        for n_dwi = 1:nscanmax
            if nscanmax == 1
                ...
                % dicom import
                sentence = sprintf('%s/connectome_scripts/dcm2niix/dcm2niix -f %s -o %s -v y %s',paths.dcm2niix,'PA_b0',paths.DWI.UNWARP,paths.DWI.dcmPA);
                [~,result] = system(sentence);
                % save log file
